...
$(document).ready(function(){
    ...
    $('#xiaohua .next').click(function(){
        loadXiaoHua();
    });

    $('#xiaohua .love').click(function(){
        xiaoHuaData['whether_like'] = !xiaoHuaData['whether_like'];

        // request to 远端
        if(xiaoHuaData['whether_like'])
            $.getJSON(siteUrl+'/app/like_joke/'+xiaoHuaData['joke_id']+'?stuid='+stuid+'&pwd='+pwd+'&token='+token+'&callback=?');
        else
            $.getJSON(siteUrl+'/app/dislike_joke/'+xiaoHuaData['joke_id']+'?stuid='+stuid+'&pwd='+pwd+'&token='+token+'&callback=?');
